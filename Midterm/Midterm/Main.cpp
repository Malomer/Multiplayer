...
EPlayerClass Class()
{
	EPlayerClass s_class;
	SPlayer classy;
	std::cout << std::endl;

	std::cout << "Select your class (press the number next to the class): " << std::endl;

	std::cout << "1 - Mage  (Magic Damage) " << std::endl;
	//magic damage means nothing lol.. 100hp 30dmg
	std::cout << "2 - Warrior   (High Health) " << std::endl;
	//150hp 10dmg
	std::cout << "3 - Rogue  (High Damage) " << std::endl;
	//70hp 45dmg
	std::cout << std::endl;

	char choose = ' ';
	std::cin >> choose;
	unsigned int Choice = choose;

	switch (Choice)
	{
	case 1:
		s_class = EPlayerClass::Mage;
		classy.m_class = s_class;
		break;
	case 2:
		s_class = EPlayerClass::Warrior;
		classy.m_class = s_class;
		break;
	case 3:
		s_class = EPlayerClass::Rogue;
		classy.m_class = s_class;
		break;
	}

	return classy.m_class;
}
...
void OnLobbyReady(RakNet::Packet* packet)
{
	...
	RakNet::RakString userName;
	bs.Read(userName);
	EPlayerClass classEnum;
	bs.Read(classEnum);

	unsigned long guid = RakNet::RakNetGUID::ToUint32(packet->guid);
	SPlayer& player = GetPlayer(packet->guid);
	player.m_name = userName;
	player.playerNum = currPlayer + 1;
	player.m_class = classEnum;
	currPlayer++;
	std::cout << userName.C_String() << " aka " << player.m_name.c_str() << " IS READY!!!!! They are Player " << player.playerNum << " and Class: " << player.m_class << std::endl;
	...
	while (isRunning)
	{
		if (g_networkState == NS_PendingStart)
		{
			...
			else
			{
				...
				RakNet::ConnectionAttemptResult car = g_rakPeerInterface->Connect("10.4.68.251", SERVER_PORT, nullptr, 0);
